...
function getPos (descriptor, spriteSheet) {
  const column = descriptor.position[0].toLowerCase().charCodeAt(0) - 97
  const row = parseInt(descriptor.position.substring(1), 10) - 1
  return {
    x: (-(spriteSheet.cellWidth + spriteSheet.padding) * column) + 'px',
    y: ((spriteSheet.cellHeight + spriteSheet.padding) * (row + 1) - spriteSheet.padding) + 'px'
  }
}

module.exports = function getIconInfos () {
  return Object.keys(Descriptors).map(key => {
    const descriptor = Descriptors[key]
    const spriteSheet = SpriteSheets[descriptor.spritesheet]
    if (!spriteSheet || descriptor.spritesheet !== 'largeicons') {
      return null
    }
    return {
      name: key,
